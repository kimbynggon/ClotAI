...
router.post('/signup', async (req, res) => {
  ...
  try {
    const existingByUserId = await User.findByUserId(user_id);
    if (existingByUserId?.is_verified) {
    ...
    let user;
    if (existingByEmail) {
      user = await User.updateUnverified(existingByEmail.id, { passwordHash, name, user_id });
    } else {
      user = await User.create({ user_id, email, passwordHash, name });
    }
    ...
    logger.info('Auth', `회원가입 완료: ${user_id} (${email})`);
    res.status(201).json({
      success: true,
      user: { id: user.id, user_id: user.user_id, email: user.email, name: user.name },
      token,
    });
  } catch (err) {
...
router.post('/login', async (req, res) => {
  ...
  try {
    ...
    const token = signToken(user.id);
    const safeUser = { id: user.id, user_id: user.user_id, email: user.email, name: user.name, is_verified: user.is_verified };
    logger.info('Auth', `로그인: ${user_id}`);
